...
static int prores_encode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->color_primaries == AVCOL_PRI_UNSPECIFIED &&
        avctx->color_transfer == AVCOL_TRC_UNSPECIFIED &&
        avctx->color_matrix == AVCOL_MTX_UNSPECIFIED) {
        if (avctx->height >= 720) {
...
static av_always_inline int quantize(DCTELEM val, int qscale)
{
    if (val < 0)
        return -((-val * qscale) >> QMAT_SHIFT);
    else
        return (val * qscale) >> QMAT_SHIFT;
}
                             ...
                             int blocks_per_slice)
{
    ...
    level = quantize(*blocks - 16384, qmat[0]);
    prev_dc = level;
    MASK_ABS(sign, level);
    ...
    for (i = 1; i < blocks_per_slice; i++, blocks += 64) {
        level = quantize(*blocks - 16384, qmat[0]) - prev_dc;
        prev_dc += level;
        MASK_ABS(sign, level);
        if (!level)
            prev_sign = 0;
                             ...
                             int blocks_per_slice)
{
    ...
    for (; pos < max_coeffs; pos++) {
        ...
        level = quantize(blocks[((pos & block_mask) << 6) + i], qmat[i]);
        if (level) {
                            ...
                            const uint8_t *src, int src_stride)
{
    ...
    for (i = 0; i < slice->mb_count; i++) {
        ctx->dsp.get_pixels(block+(0<<6), src, src_stride);
        ctx->dsp.get_pixels(block+(1<<6), src+16, src_stride);
        ctx->dsp.get_pixels(block+(2<<6), src+8*src_stride, src_stride);
        ctx->dsp.get_pixels(block+(3<<6), src+8*src_stride+16, src_stride);
        ctx->dsp.fdct(block+(0<<6));
        ctx->dsp.fdct(block+(1<<6));
        ctx->dsp.fdct(block+(2<<6));
        ctx->dsp.fdct(block+(3<<6));
        block += 4*64;
        src += 32;
    }
}
                              ...
                              int log2_blocks_per_mb)
{
    ...
    for (i = 0; i < slice->mb_count; i++) {
        for (j = 0; j < log2_blocks_per_mb; j++) {
            ctx->dsp.get_pixels(block+(0<<6), src, src_stride);
            ctx->dsp.get_pixels(block+(1<<6), src+8*src_stride, src_stride);
            ctx->dsp.fdct(block+(0<<6));
            ctx->dsp.fdct(block+(1<<6));
            block += 2*64;
            src += 16;
...
static int prores_write_frame_header(AVCodecContext *avctx, uint8_t *buf)
{
    ...
    buf[15] = avctx->color_transfer; // color transfer
    buf[16] = avctx->color_matrix; // color matrix;
...
static const AVOption options[] = {
    {"cqp", "Set quantization parameter", OFFSET(qp), FF_OPT_TYPE_INT, {.dbl=0}, 0, 224, VE},
    {"b", "Set bit rate in (bits/s)", OFFSET(bitrate), FF_OPT_TYPE_INT64, {.dbl=0}, 0, INT_MAX, VE},
    {"ratetol", "Set bit rate tolerance in %", OFFSET(bt), FF_OPT_TYPE_FLOAT, {.dbl=5}, 0, INT_MAX, VE},
    {"profile", "Set encoding profile: proxy,lt,std,hq", OFFSET(profile), FF_OPT_TYPE_STRING, {.str=0}, 0, CHAR_MAX, VE},
    { NULL }
};
